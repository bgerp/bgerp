...
class acc_BalanceDetails extends core_Detail
{
    ...
    public function description()
    {
        ...
        $this->setDbIndex('accountId,ent1Id');
        $this->setDbIndex('accountId,ent2Id');
    ...
    private function setGroupingForField($i, $listRec, &$form, $options)
    {
        ...
        $form->FNC("feat{$i}", 'varchar', "maxRadio=0,silent,caption={$listName}->Свойства,width=330px,input,class=balance-feat");
        if (countR($options)) {
                        ...
                        if (!empty($lastItem)) {
                            ...
                            if (cls::haveInterface('acc_RegistryDefaultCostIntf', $itemRec->classId)) {
                                ...
                                if (acc_Accounts::hasStrategy($rec->{"{$type}AccId"})) {
                                    ...
                                    $defCost = $Register->getDefaultCost($itemRec->objectId, $rec->{$quantityField}, $rec->valior);
