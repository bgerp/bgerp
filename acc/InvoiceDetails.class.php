...
class acc_InvoiceDetails extends core_Detail
{
    ...
    var $loadList = 'plg_Created, plg_RowTools, 
                          ...
                          EmployeesList=catering_EmployeesList, Contacts=contacts_Contacts, 
    ...
    function on_AfterPrepareEditForm($mvc, $res, $data)
    {
        ...
        if (!haveRole('admin,catering')) {
            ...
            while($recEmployeesList = $queryEmployeesList->fetch("1=1")) {
                $selectOptEmployeesList[$recEmployeesList->id] = $mvc->Contacts->fetchField($recEmployeesList->contactId, 'name');
            }
        }
        ...
        foreach($menuArr as $k => $v) {
            ...
            while($rec = $queryMenuDetails->fetch("#menuId   = {$k}")) {
                ...
                $menuDetailsArr[$rec->id]->companyIdContacts = $mvc->Companies->fetchField("#id = {$menuDetailsArr[$rec->id]->companyId}", 'companyId');
                $menuDetailsArr[$rec->id]->companyName = $mvc->Contacts->fetchField("#id = {$menuDetailsArr[$rec->id]->companyIdContacts}", 'name');
    ...
    function on_AfterRecToVerbal($mvc, $row, $rec)
    {
        ...
        $contactId = $mvc->EmployeesList->fetchField($rec->personId, 'contactId');
        $personName = $mvc->Contacts->fetchField($contactId, 'name');
        
        if ($personId == $rec->personId) {
        ...
        $companyIdContacts = $mvc->Companies->fetchField($companyId, 'companyId');
        $row->companyName = $mvc->Contacts->fetchField($companyIdContacts, 'name');
