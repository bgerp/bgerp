...
class acc_Journal extends core_Master
{
    ...
    /**
     * Валидира дебитната или кредитната страна или и двете на ред от счет. транзакция.
     * 
     * Валидацията включва:
     *   * дали е зададена коректно сметка
     *   * дали зададените пера пасват по брой, позиция и интерфейс на разбивките на сметката 
     * 
     * @param stdClass $entry
     * @param string $type NULL, 'debit' или 'credit' - NULL - валидира и двете страни на реда
     */
    protected static function validateEntry($entry, $type = NULL)
    {
        if (!isset($type)) {
            return static::validateEntry($entry, 'debit') 
                && static::validateEntry($entry, 'credit');  
        }
        
        expect(($type = strtolower($type)) && ($type == 'debit' || $type == 'credit'));
        
        expect($accountRec = static::fetchAccount($entry, $type));
        
        // Проверка дали са зададени пера по всички разбивки на сметката и дали (мениджърите на)
        // тези пера поддържат интерфейса, зададен в съотв. номенклатура-разбивка. 
        foreach (range(1, 3) as $i) {
            // Полагания, за краткост
            $itemId = &$entry->{"{$type}Item{$i}Id"}; 
            $item   = &$entry->{"{$type}Item{$i}"};
            $listId = $accountRec->{"groupId{$i}"};
            
            // липсата на $i-то перо е валидна точно тогава, когато сметката няма $i-та разбивка
            // наличието на $i-то перо е валидно точно тогава, когато сметката има $i-та разбивка
            expect(!(empty($listId) xor (empty($itemId) && empty($item))));
            
            if (empty($listId)) {
                // Няма $i-та разбивка, не е зададено и $i-то перо. Всичко е наред
                // Продължаваме с останалите пера
                continue;
            }

            if (!empty($item->cls)) {
                // Нормализация - класът на перото като число
                $item->cls = core_Classes::getId($item->cls);
            }
            
            if (!empty($itemId)) {
                expect($itemRec = acc_Items::fetch($itemId));
                expect(empty($item) || ($item->cls == $itemRec->classId && $item->id == $itemRec->objectId));
            ...
            expect(!empty($item->cls) && !empty($item->id), $item);
            
            // Проверка дали мениджъра на перото $item поддържа интерфейса на номенклатурата 
            // $i-та разбивка на сметката
            expect($interfaceId = acc_Lists::fetchField($listId, 'regInterfaceId'));
            expect($interfaceName = core_Interfaces::fetchField($interfaceId, 'name'));
            expect(cls::haveInterface($interfaceName, $item->cls));

            $item->listId = $listId;
            
            //
            // Браво, $i-тото перо е валидно!
            //
        }
        
        // Всичко с ($type-частта на) реда е наред
        return TRUE;
    }
    
    
    /**
     * Извлича дебитната / кредитната сметка на запис от транзакция
     * 
     * @param stdClass $entry
     * @param string $type debit или credit 
     * @return stdClass запис на модела acc_Accounts
     */
    protected static function fetchAccount($entry, $type)
    {
        expect(($type = strtolower($type)) && ($type == 'debit' || $type == 'credit'));
        
        $id       = &$entry->{"{$type}AccId"};
        $systemId = &$entry->{"{$type}Acc"};
        
        // Извличаме записа на сметка по id или systemId
        expect($accountRec = acc_Accounts::fetchById($id, $systemId));
        
        $id       = $accountRec->id;
        $systemId = $accountRec->systemId;
