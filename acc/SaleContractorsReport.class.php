 ...
 * @since     v 0.1
 */
class acc_SaleContractorsReport extends frame_BaseDriver
{
    ...
    public function renderEmbeddedData($data)
    {
    	...
    	$tpl = new ET("
            <h1>Приходи на продажби по клиенти</h1>
            [#FORM#]
    	...
    	$f->FLD('contractor', 'class(interface=doc_DocumentIntf,select=title,allowEmpty)', 'caption=Контрагент');
    	...
    	foreach ($data->recs as $accountsId => $rec) {
    		...
    		$contractor = substr($accountsId,1 + strlen($rec->accountId) ,-6 );
    		$deal = substr($accountsId,2 + strlen($rec->accountId) + strlen($contractor) ,-3 );
    		$row = new stdClass();
    		
    		
    		foreach (range(1,3) as $i) {
	    		if(!empty($rec->{"ent{$i}Id"})){
	    			    $row->contractor = acc_Items::getVerbal($rec->{"ent1Id"}, 'titleLink');
    	...
    	$html = $table->get($rows, 'contractor=Контрагент,baseAmount=Начално салдо->ДК,debitAmount=Обороти->Дебит,creditAmount=Обороти->Кредит,blAmount=Крайно салдо->ДК');
