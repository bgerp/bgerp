...
class acc_reports_PurchasedProducts extends acc_reports_CorespondingImpl
{
    ...
    public static function on_AfterPrepareListFields($mvc, &$res, $data)
    {
        ...
        if($mvc->innerForm->compare != 'no'){
    		...
    		$fields = arr::make("item2=Контрагенти,item3=Артикул,blQuantity={$prefix}->Количество,blAmount={$prefix}->Сума,delta={$prefix}->Дял,blQuantityNew={$prefixOld}->Количество,blAmountNew={$prefixOld}->Сума,deltaNew={$prefixOld}->Дял", TRUE);
