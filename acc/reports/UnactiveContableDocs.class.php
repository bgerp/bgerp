...
class acc_reports_UnactiveContableDocs extends frame2_driver_TableData {
	...
	protected function detailRecToVerbal($rec, &$dRec) {
		...
		$row->documentType .= "<span class= 'small' >" . "$typeOfDocument" . $Date->toVerbal ( $typeOfDate ) . "</span>" . '  »  ' . "<span class= 'state-{$state} document-handler' >" . ht::createLink ( "#{$handle}", $singleUrl, FALSE, "ef_icon={$Document->singleIcon}" ) . "</span>";
		
		$row->valior = $Date->toVerbal ( $dRec->valior );
		
		$row->states = "<span class= 'state-{$state} document-handler' >" . ($dRec->states) . "</span>";
