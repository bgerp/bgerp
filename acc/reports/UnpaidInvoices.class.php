...
class acc_reports_UnpaidInvoices extends frame2_driver_TableData {
	...
	public function addFields(core_Fieldset &$fieldset) {
		$fieldset->FLD ( 'contragent', 'key2(mvc=doc_Folders,select=title,allowEmpty, restrictViewAccess=yes,coverInterface=crm_ContragentAccRegIntf)', 'caption=Контрагент,mandatory,single=none,after=title' );
			...
			foreach ( $threadsId as $thread ) {
				...
				if (is_array ( $invoicePayments )) {
					...
					foreach ( $invoicePayments as $inv => $paydocs ) {
						...
						$iRec = $Invoice->fetch ( 'id,number,dealValue,discountAmount,vatAmount,rate,type,originId,containerId,currencyId,date,dueDate' );
	...
	protected function detailRecToVerbal($rec, &$dRec) {
		...
		$row->invoiceNo = ht::createLinkRef ( $invoiceNo, array (
				$dRec->className,
				'single',
				$dRec->invoiceId 
		) );
