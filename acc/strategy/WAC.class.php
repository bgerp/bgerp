...
/**
 * Клас 'acc_strategy_WAC' - за средно претеглена цена
 *
 *
 * @category  bgerp
 * @package   acc
...
class acc_strategy_WAC extends acc_strategy_Strategy
{
    ...
    /**
     * Връща среднопретеглената сума към дадена дата на дадена аналитична сметка
     * за подадено количество.
     *
     * Връща среднопретеглената сума от началото на месеца на подадената дата до подадената дата.
     ...
     *
     * @return mixed $amount   - сумата за количеството спрямо среднопретеглената цена
     */
    public static function getAmount($quantity, $date, $accSysId, $item1, $item2, $item3, $maxTries = null, &$currentTry = null)
    {
        ...
        if ($balanceRec = cls::get('acc_Balances')->getBalanceBefore($from)) {
            ...
            while ($bRec = $bQuery->fetch()) {
                ...
                if ($bRec->blQuantity >= 0 || Mode::is('feedStrategyWithNegativeQuantity')) {
