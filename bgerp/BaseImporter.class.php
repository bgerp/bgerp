...
class bgerp_BaseImporter extends core_Manager
{
    ...
    public function getFields()
    {
        ...
        foreach ($Dfields as $name => $fld) {
            if ('none' != $fld->input && 'hidden' != $fld->input &&
                'FNC' != $fld->kind && !($fld->type instanceof type_Enum) &&
    ...
    public function import($rows, $fields)
    {
        ...
        foreach ($rows as $row) {
            ...
            if (false === $this->mvc->invoke('BeforeImportRec', array(&$rec))) {
                $errArr[] = $row;
