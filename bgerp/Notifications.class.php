...
class bgerp_Notifications extends core_Manager
{
    ...
    public static function add($msg, $urlArr, $userId, $priority = null, $customUrl = null, $addOnce = false)
    {
        ...
        if (is_object($r)) {
            if ($addOnce && ('active' == $r->state) && ('no' == $r->hidden) && ($r->msg == $rec->msg) && ($r->priority == $rec->priority)) {
    ...
    public static function on_AfterRecToVerbal($mvc, $row, $rec)
    {
        ...
        if ('active' == $rec->state) {
            ...
            if ('alert' == $rec->priority) {
                $attr['style'] .= 'color:#cc0033 !important;';
            } elseif ('warning' == $rec->priority) {
    ...
    protected static function getAutoNotifyArr($rec, $update = null, &$haveStopped = null)
    {
        ...
        if (false !== strpos($msg, '|отворени теми в|')) {
            $stopNotifyArr['folOpenings'] = 'doc_Folders';
        } elseif ((false !== strpos($msg, '|добави|')) || (false !== strpos($msg, '|хареса')) || (false !== strpos($msg, '|промени|'))) {
            if (false !== strpos($msg, '|входящ имейл|')) {
        ...
        } elseif (false !== strpos($msg, '|създаде заявка за|')) {
                ...
                if ($plugins['planning_plg_StateManager']) {
                    if ($nActArr = $doc->instance->notifyActionNamesArr) {
                        foreach ($nActArr as $actName) {
                            ...
                            if (false !== strpos($msg, "|{$actName} на|* ")) {
        ...
        foreach ($stopNotifyArr as $kVal => $kClass) {
            ...
            if (!$valsArr[$key][$kVal] || ('no' != $valsArr[$key][$kVal])) {
    ...
    public static function on_AfterPrepareListFilter($mvc, $data)
    {
        ...
        if ('show' == strtolower(Request::get('Act'))) {
            ...
            if ($filter = $data->listFilter->rec) {
                ...
                if ((false !== strpos($filter->usersSearch, '|-1|')) && (haveRole('ceo'))) {
                    // Търсим всичко
                } else {
        ...
        while ($rec = $query->fetch()) {
            ...
            try {
                ...
                if ((!cls::load($ctr, true)) || ($urlArr['id'] && !($cRec = $ctr::fetch($urlArr['id'])))) {
                    ...
                    if (!$haveRight && ('no' == $rec->hidden)) {
                        $rec->hidden = 'yes';
                        self::save($rec, 'hidden,modifiedOn,modifiedBy');

                        self::logInfo('Скрит недостъпен ресурс', $rec->id);
                    } elseif ($haveRight && ('yes' == $rec->hidden)) {
                        if (!$rec->cnt) {
                            continue;
                        }

                        if (!$cRec || ('rejected' == $cRec->state)) {
                            continue;
                        }
