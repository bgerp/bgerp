...
class cal_Reminders extends core_Master
{
    ...
    public $loadList = ' cal_Wrapper, plg_Clone,doc_DocumentPlg, plg_RowTools2, plg_Printing, doc_ActivatePlg, doc_SharablePlg, 
    				  bgerp_plg_Blank, plg_Sorting, plg_State, change_Plugin,doc_plg_Close,doc_plg_SelectFolder,plg_Search,doc_plg_TxtExportable';
    ...
    public function description()
    {
        ...
        // Споделяне
        $this->FLD('sharedUsers', 'userList(showClosedUsers=no)', 'caption=Споделяне->Потребители,changable,silent');
                ...
                if ($userId > 0 && doc_Threads::haveRightFor('single', $rec->threadId, $userId)) {
                    ...
                    switch ($rec->action) {
                        ...
                        case 'threadOpen':
                            ...
                            $rToSave =(object) array('id' => $rec->threadId, 'state' => 'opened');
                            doc_Threads::save($rToSave, 'state');
            ...
            if (isset($rec->repetitionEach, $rec->repetitionType)) {
                if (isset($rec->timePreviously)) {
                    ...
                    if ($rec->timePreviously > $repetitionSec) {
                        ...
                        if ($rec2->timeStart < dt::now()) {
                            $rec2->timeStart = $rec2->calcTimeStart;
                        }
                    }
                }
            }
