...
class callcenter_Numbers extends core_Manager
{
    ...
    /**
     * Връща опциите за избиране на контрагентите
     * 
     * @return array $contragentOptions - Масив с опциите
     */
    static function getContragentOptions()
    {
        // Данни за кеширане
        $cacheType = 'callcenterNumbers';
        $cacheHandle = 'allNumbers';
        $keepMinutes = 1000;
        $depends = array('email_Inboxes', 'callcenter_Numbers');
        
        // Ако няма в кеша или е променен
        if (!$contragentOptions = core_Cache::get($cacheType, $cacheHandle, $keepMinutes, $depends)) {
            
            // Опциите, които ще връщаме
            $contragentOptions[''] = '';
            
            // Вземаме всички записи, подредени по класове
            $query = static::getQuery();
            $query->orderBy('classId');
            
            // Обхождаме резултатите
            while ($rec = $query->fetch()) {
                
                // Ако няма клас
                if (!$rec->classId) continue;
                
                // Инстанция на класа
                $ClassInst = cls::get($rec->classId);
                
                // Ако класа не е добавен
                if (!$contragentOptions[$rec->classId]) {
                    
                    // Нов обект
                    $classOpt = new stdClass();
                    
                    // Попълваме данните
                    $classOpt->title = $ClassInst->title;
                    $classOpt->attr = array('class' => 'team');
                    $classOpt->keylist = $rec->classId;
                    
                    // Добавяме в масива
                    $contragentOptions[$rec->classId] = $classOpt;
                    
                    // Изтриваме инстанцията
                    unset($classOpt);
                }
                
                // Името на ключа на полето
                $optionKeyName = $rec->classId . '_' . $rec->contragentId;
                
                // Ако е инстанция на профили
                if (($ClassInst instanceof crm_Profiles)) {
                    
                    // Вземаме името на потребителя
                    $contragentOptions[$optionKeyName] = $ClassInst->getVerbal($rec->contragentId, 'userId');
                } else {
                    
                    // Вземаме името на визитката
                    $contragentOptions[$optionKeyName] = $ClassInst->getVerbal($rec->contragentId, 'name');
                }
            ...
            // Записваме в кеша
            core_Cache::set($cacheType, $cacheHandle, $contragentOptions, $keepMinutes, $depends);
        }
        
        return $contragentOptions;
    }
}
