...
//        $this->haveRightFor('single', $rec);
    ...
    public static function on_AfterRecToVerbal($mvc, &$row, &$rec, $fields)
    {
        ...
        if (isset($fields['thumb'])) {
            $attr = array();
            if ($driver->isActive()) {
                $attr['src'] = toUrl(array($mvc, 'ShowImage', $rec->id, 'thumb' => 'yes'));
                $attr['class'] = 'camera-tumb';
                $row->thumb = ht::createLink(ht::createElement('img', $attr), array($mvc, 'Single', $rec->id));
            } else {
        ...
        $attr = array();
        $url = toUrl(array($mvc, 'ShowImage', $rec->id));
        $attr['src'] = $url;
