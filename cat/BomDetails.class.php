            ...
            if($rec->type == 'stage'){
                if(cat_BomDetails::count("#parentId = {$rec->id} AND #bomId = {$rec->bomId}")){
                    ...
                    if(in_array($bomRec->state, array('active', 'closed'))){
                        ...
                        $extraBtnTpl2->replace(" <a href=\"javascript:toggleDisplayBomStepDetails('{$position}', 'bomExpandStageDetails{$rec->id}');\"  style=\"background-image:url(" . sbf('img/16/minus-black.png', "'") . ');" data-id="' .$rec->id. 'inf" class=" plus-icon more-btn show-btn newIconStyle bomExpandStageDetails' . $rec->id . '" title="Показване/Скриване на детайли"> </a>', 'BTN');
