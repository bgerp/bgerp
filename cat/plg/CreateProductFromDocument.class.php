...
class cat_plg_CreateProductFromDocument extends core_Plugin
{
    ...
    public static function on_BeforeAction($mvc, &$tpl, $action)
    {
        if ($action == 'createproduct') {
            ...
            if($form->getFieldParam('packPrice', 'mandatory')){
                $form->setField('packPrice', 'caption=Цена||Price');
            }
            ...
            if ($form->isSubmitted()) {
                ...
                if(!($mvc instanceof store_InternalDocumentDetail)){
                    ...
                    if($setWarning == 'yes'){
                        if(deals_Helper::fetchExistingDetail($mvc, $rec->{$mvc->masterKey}, $rec->id, $productId, $rec->packagingId, $rec->price, $rec->discount, $rec->tolerance, $rec->term, $rec->batch, null, $rec->notes, $rec->quantity)){
                            $form->setWarning('productId,packagingId,packPrice,discount,notes,packQuantity', 'Вече има запис със същите данни');
                        }
                    }
                }
                ...
                if (!$form->gotErrors()) {
                    ...
                    if(!($mvc instanceof purchase_QuotationDetails)){
                    ...
                    if (isset($d->costItemId)) {
                        ...
                        $saveRec = (object) array('detailClassId' => $mvc->getClassId(), 'detailRecId' => $dRec->id, 'productId' => $productId, 'expenseItemId' => $d->costItemId, 'containerId' => $masterRec->containerId, 'quantity' => $dRec->quantity, 'allocationBy' => 'auto', 'allocationFilter' => 'storable');

                        $itemRec = acc_Items::fetch($d->costItemId, 'classId,objectId');
                        $origin = new core_ObjectReference($itemRec->classId, $itemRec->objectId);
                        $saveRec->productsData = $origin->getCorrectableProducts($mvc->Master, 'storable');
