            ...
            if ($Type = cat_Params::getTypeInstance($rec->paramId, $rec->classId, $rec->productId, $rec->paramValue)) {
                ...
                if($pRec->valueType == 'readonly'){
                    if(isset($defaultValue)){
                        $form->info = tr("Параметърът е дефиниран като "Само за четене"|*!<br>|Промяната наложителна ли е|*?<br>Съвет|*: |Опитайте първо да презапишете с автоматично заредената дефолтна стойност|*!");
                    }
                    else {
                        $form->info = tr("Параметърът е дефиниран като "Само за четене"|*!<br>|Промяната наложителна ли е|*?");
                    }
                }
