function prepareChart( idElem, chartData, charType) {
    ...
    var fillColor = ["rgba(10,161,255,0.5)", "rgba(252,162,53,0.5)", "rgba(112,193,15,0.5)","rgba(255,33,23,0.5)", "rgba(51,254,224,0.5)",
        "rgba(200,200,200,0.5)" ,"rgba(34,101,10,0.5)", "rgba(141,12,201,0.5)","rgba(247,247,17,0.5)", "rgba(50,35,183,0.5)","rgba(99,41,22,0.5)", "rgba(203,246,144,0.5)", "rgba(203,144,,246,0.5)"];
    ...
    if (charType == "pie") {
        ...
        var myNewChart = new Chart(pie).Pie(data, {
            ...
            tooltipTemplate:  "<%=label%>: <%= numeral(value).format('(00[.]00)') %>" + suffix + " (<%= numeral(circumference / 6.283).format('(0[.][00]%)') %>)"  ,
            ...
            "<%if(segments[i].label){%><%=segments[i].label%><%}%>: <%if(segments[i].value){%><%= numeral(segments[i].value).format('(00[.]00)') %>"+ suffix + " (<%= numeral(segments[i].circumference / 6.283).format('(0[.][00]%)') %>)<%}%></li><%}%></ul>"
        });
        // добавяме легендата
