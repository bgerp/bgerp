function prepareChart(animation) {
    ...
    $('.diagramCanvas').each(function(){
        var chartData = JSON.parse($(this).attr("data-data"));
        var charType =  $(this).attr("data-type");

        if (!chartData) return;
        var animationDuration = (animation) ? 1000:0;
        // ако чертаем торта
        if (charType == "pie") {
