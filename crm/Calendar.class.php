...
class cal_Agenda extends core_Master
{
    ...
    static function updateEventsPerObject($caller, $objectId)
    {
        ...
        if(count($events)) {
            foreach($events as $eRec) {
                $eRec->id = cal_Agenda::fetchField("#date = '{$eRec->date}' AND #type = '{$eRec->type}' AND #classId = {$classId} AND #objectId = {$objectId}", 'id');
                $eRec->classId = $classId;
                $eRec->objectId = $objectId;
                
                if(!$eRec->id) {
                    cal_Agenda::save($eRec);
                    $eventsCnt++;
                }
            ...
            cal_Agenda::delete("#classId = '{$classId}' AND #objectId = {$objectId} AND NOT(#id IN ({$idList}))");
        }
    ...
    static function deleteEventsPerObject($caller, $objectId)
    {
        ...
        // Изтриване на събитията до момента
        $eventsCnt = cal_Agenda::delete("#classId = '{$classId}' AND #objectId = {$objectId}");
