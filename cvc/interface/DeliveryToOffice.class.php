...
/**
 * Драйвер за доставка до Офис на CVC
 *
 * @category  bgerp
 * @package   cvc
 *
 ...
 * @since     v 0.1
 */
class cvc_interface_DeliveryToOffice extends core_BaseClass
{
    /**
    ...
    {
        $form->FLD('officeId', "key(mvc=cvc_Offices,select=name, allowEmpty)", 'silent,mandatory,caption=Доставка->Офис');
    ...
    public function getVerbalDeliveryData($termRec, $deliveryData, $document)
    {
        ...
        if($deliveryData['officeId']){
            $officeRec = cvc_Offices::fetch($deliveryData['officeId']);
