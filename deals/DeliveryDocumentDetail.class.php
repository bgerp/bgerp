	...
	protected static function inputDocForm(core_Mvc $mvc, core_Form $form)
	{
		...
		if($form->rec->productId){
			...
			if(isset($mvc->$LastPolicy)){
				$policyInfoLast = $mvc->$LastPolicy->getPriceInfo($masterRec->contragentClassId, $masterRec->contragentId, $rec->productId, cat_Products::getClassId(), $rec->packagingId, $rec->packQuantity, $masterRec->valior, $masterRec->currencyRate, $masterRec->chargeVat);
		...
		if ($form->isSubmitted() && !$form->gotErrors()) {
			...
			if (!isset($rec->packPrice)) {
				...
				if(!$policyInfo){
					...
					$policyInfo = $Policy->getPriceInfo($masterRec->contragentClassId, $masterRec->contragentId, $rec->productId, cat_Products::getClassId(), $rec->packagingId, $rec->packQuantity, $masterRec->valior, $masterRec->currencyRate, $masterRec->chargeVat);
				}
