...
abstract class deals_InvoiceMaster extends core_Master
{
    ...
    public function getInvoiceDetailedInfo($containerId, $applyDiscount = false)
    {
        ...
        while ($dRec = $query->fetch()) {
            ...
            $key1 = "{$dRec->productId}|{$dRec->packagingId}|{$dRec->quantityInPack}|{$dRec->batches}|Q{$dRec->quantity}";
            $key2 = "{$dRec->productId}|{$dRec->packagingId}|{$dRec->quantityInPack}|{$dRec->batches}|P{$price}";
