...
class deals_plg_SaveValiorOnActivation extends core_Plugin
{
    ...
    {
        $valiorToBe = $mvc->getFieldType($mvc->valiorFld)->toVerbal(dt::today());
        $row->{$mvc->valiorFld} = (isset($rec->{$mvc->valiorFld})) ? $row->{$mvc->valiorFld} : ((Mode::is('printing')) ? $valiorToBe : ht::createHint("<span style='color:blue'>{$valiorToBe}</span>", 'Вальорът ще бъде записан при контиране|*!'));
    }
