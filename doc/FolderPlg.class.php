...
class doc_FolderPlg extends core_Plugin
{
    ...
    function on_AfterDescription(&$mvc)
    {
        if(!$mvc->fields['folderId']) {
            ...
            $mvc->FLD('access', 'enum(team=Екипен,private=Личен,public=Общ,secret=Секретен)', 'caption=Права->Достъп,formOrder=10000,notNull,value=' . $defaultAccess);
            $mvc->FLD('shared' , 'userList', 'caption=Права->Споделяне,formOrder=10000');
        }
