...
class doc_FolderPlg extends core_Plugin
{
    ...
    public static function on_AfterGetRequiredRoles($mvc, &$requiredRoles, $action, $rec = null, $userId = null)
    {
        ...
        if (!$requiredRoles || $requiredRoles == 'powerUser' || $requiredRoles == 'user') {
            if (is_object($rec) && !empty($rec->id) && ($action == 'delete' || $action == 'edit' || $action == 'write' || $action == 'close' || $action == 'reject')) {
                if (!$userId) {
    ...
    public static function on_AfterForceCoverAndFolder($mvc, &$folderId, &$rec, $bForce = true)
    {
        if (!$folderId) {
            ...
            if ($bForce && (!($rec->folderId ?? null) || !doc_Folders::fetch($rec->folderId))) {
                ...
                expect(countR((array) $rec), 'Опит за създаване на празна корица');
