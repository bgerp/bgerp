...
class doc_plg_TplManagerDetail extends core_Plugin
{
    ...
    {
        if (!empty($rec->id) && $rec->{$mvc->masterKey}) {
            $Master = $mvc instanceof core_Detail ? $mvc->getMasterMvc($rec) : $mvc->Master;
            $template = $Master->getTemplate($rec->{$mvc->masterKey});
            $rec->tplLang = $Master->pushTemplateLg($template);
        }
    }
    ...
    public static function on_AfterPrepareListFields(core_Mvc $mvc, &$data)
    {
        ...
        if (countR($toggleFields) && ($toggleFields[$mvc->className] ?? null) !== null) {
