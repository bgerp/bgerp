...
class doclog_Documents extends core_Manager
{
     ...
     */
    public $listFields = 'createdOn, createdBy, action=Какво, containerId=Кое, dataBlob';
...
//         $this->FLD('baseParentId', 'key(mvc=doclog_Documents, select=action)', 'input=none,caption=Основание');
     ...
     */
    public function on_CalcData($mvc, $rec)
    {
        // Вземаме dataBlob
     ...
     */
    public function on_CalcReceivedOn($mvc, $rec)
    {
     ...
     */
    public function on_CalcReturnedOn($mvc, $rec)
    {
     ...
     * @param object $data
     */
    public function prepareForward($data)
    {
        ...
        foreach ($dataRecsArr as $forwardRec) {
            ...
            // Записите
            $row = (object) array(
                'time' => $forwardRec['on'] ?? null,
                'from' => $forwardRec['from'] ?? null,
            );
            ...
            // Вземаме документите
            $forwardContainerId = $forwardRec['containerId'] ?? null;
            if (!$forwardContainerId) {
                continue;
            }
            $doc = doc_Containers::getDocument($forwardContainerId);
    ...
    protected function getRecsForPaging(&$data, $recs, $action)
    {
        ...
        foreach ($allDataAct as $val) {
            if (isset($data->pager->rangeStart, $data->pager->rangeEnd)) {
                $curr++;
                
                if ($curr <= $data->pager->rangeStart) {
                    continue;
                }
     ...
     * @param object $data
     */
    public function preparePrint($data)
    {
        // Ако сме в режим принтиране
        // Да не се изпълнява
        if (Request::get('Printing')) {
     ...
     * @param object $data
     */
    public function prepareOpen($data)
    {
        // Ако сме в режим принтиране
        // Да не се изпълнява
        if (Request::get('Printing')) {
        ...
        foreach ($nRecsArr as $o) {
            ...
            $row = (object) array(
                'time' => $o['on'] ?? null,
                'ip' => $o['ip'] ?? null,
                'openAction' => isset($o['ParentRec']) ? static::formatViewReason($o['ParentRec']) : null,
            );
     ...
     * @param object $data
     */
    public function prepareSend($data)
    {
        // Ако сме в режим принтиране
        // Да не се изпълнява
        if (Request::get('Printing')) {
        ...
        foreach ($recs as $i => $rec) {
            ...
            if (!empty($rec->data->faxTo)) {
                ...
                $row->service = $rec->data->service ?? null;
            }
            ...
            if (!empty($row->returnedOn)) {
                ...
                if (!empty($rec->data->returnedIp)) {
                    $returnedStr .= ' ' . type_Ip::decorateIp($rec->data->returnedIp, $rec->data->returnedOn ?? null, true);
                }
     ...
     * @param object $data
     */
    public function prepareDownload($data)
    {
        // Ако сме в режим принтиране
        // Да не се изпълнява
        if (Request::get('Printing')) {
        ...
        foreach ($recs as $key => $rec) {
            foreach ((array) ($rec->data->{$action} ?? array()) as $fh => $rArr) {
                foreach ((array) $rArr as $dArr) {
                    $dArr['fileHnd'] = $fh;
        ...
        foreach ($dataRecsArr as $downData) {
            ...
            $nRec = (object) array(
                'time' => $downData['seenOnTime'] ?? null,
                'from' => $downData['seenFrom'] ?? null,
                'ip' => $downData['ip'] ?? null,
            );
            ...
            $row->fileHnd = isset($downData['fileHnd']) ? fileman_Files::getLink($downData['fileHnd']) : null;
     ...
     * @param object $data
     */
    public function prepareChanged($data)
    {
        // Ако сме в режим принтиране
        // Да не се изпълнява
        if (Request::get('Printing')) {
     ...
     * @return core_ET
     */
    public static function getLogDetailTpl()
    {
        // Шаблона
     ...
     *
     * @param int|NULL $cid    - containerId
     * @param mixed    $action - Масив или стринг с екшъна
     *
     * @return array $recsArr - Масив с намерените записи
     */
    public static function getRecs($cid = null, $action = null, $threadId = null, &$pager = null)
    {
        // Очакваме да има $cid
        expect($cid || $threadId);
     ...
     * @param string $mid
     *
     * @return object|bool запис на модела или FALSE
     *
     */
    ...
    protected static function buildThreadHistory($threadId)
    {
        ...
        while ($rec = $query->fetch()) {
            ...
            if ($rec->action == $change) {
                ...
                foreach ((array) ($rec->data->{$change} ?? array()) as $changeDataArr) {
                    if (empty($changeDataArr['docClass']) || empty($changeDataArr['docId'])) {
                        continue;
                    }
     ...
     *
     * @return int $downloadCount - Броя на свалянията на файловете
     */
    ...
    private static function renderOpenActions($rec, $date = null, $brief = true)
    {
        ...
        if (isset($firstOpen['on']) && (empty($date) || $firstOpen['on'] < $date)) {
            $rec->receivedOn = $firstOpen['on'] ?? $date;
        } else {
            $rec->receivedOn = $date;
        }
        ...
        if (!empty($firstOpen) && isset($firstOpen['on'])) {
            $html .= ' ' . type_Ip::decorateIp($firstOpen['ip'] ?? null, $firstOpen['on'], true);
     ...
     */
    public static function getDocumentCidFromURL($url)
    {
     ...
     */
    public static function getActionRecForMid($mid, $action = null)
    {
     ...
     *
     * @param int          $containerId   - id на контейнера
     * @param int|NULL     $resendingSecs - Секунди, преди които ще се счита за изпратено
     * @param string|FALSE $emailTo       - Имейли в to
     * @param string|NULL  $emailCc       - Имейли в cc
     *
     * @return bool
     */
    public static function isSended($containerId, $resendingSecs = null, $emailTo = false, $emailCc = null)
    {
        ...
        while ($rec = $query->fetch()) {
            ...
            if ($emailTo !== false) {
                ...
                if ($emailCc) {
                    ...
                    if ((($rec->data->cc ?? null) == $emailCc) && (($rec->data->to ?? null) == $emailTo)) {
     ...
     */
    public function prepareUsed($data)
    {
        // Ако сме в режим принтиране
        // Да не се изпълнява
        if (Request::get('Printing')) {
     ...
     * @param bool         $isRejected - Дали документа се оттегля или не
     */
