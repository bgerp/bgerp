...
class drdata_ParseBGAddress extends core_Manager
{
    ...
    public static function parse($str)
    {
        ...
        $str = preg_replace_callback("/([^a-zа-я][aeotkm][а-я]{1,30}[^a-zа-я])/iu", function($a) { return  str_ireplace(array('a', 'e', 'o', 't', 'k', 'm'), array('А', 'Е', 'О', 'Т', 'К', 'М'), $a[1]);}, $str);
        ...
        $str = preg_replace_callback("/([^a-zа-я\" ]) ?- ?([a-zа-я]{2,10})([^a-zа-я])/iu", function($a) {  return  rtrim($a[1]) . '-' . ltrim(mb_strtolower($a[2])) . $a[3];}, $str);
        
        // Думи, които трябва да са с малки букви
        $str = preg_replace_callback("/([^a-zа-я])(инж\.?|проф\.?|пор\.?|акад\.?|ген\.?|д-р\.?|м-р\.?|в\/у|и)([^a-zа-я])/iu", function($a) {  return  $a[1] . mb_strtolower($a[2]) . $a[3];}, $str);
