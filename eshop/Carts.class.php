...
class eshop_Carts extends core_Master
{
    ...
    public function description()
    {
        ...
        $this->FLD('locationId', 'key(mvc=crm_Locations,select=title)', 'caption=Доставка->Локация,input=none,silent,removeAndRefreshForm=deliveryData|deliveryCountry|deliveryPCode|deliveryPlace|deliveryAddress|routeId,after=termId');
    ...
    {
        $data->listFilter->FNC('domain', 'key(mvc=cms_Domains,select=titleExt,allowEmpty)', 'caption=Домейн,input,refreshForm,placeholderType=all');
    ...
    private static function sendEmail($rec, &$saleRec)
    {
        ...
        if (isset($rec->termId)) {
            ...
            if ($Driver = cond_DeliveryTerms::getTransportCalculator($rec->termId)) {
                ...
                $deliveryCountry = '';
                if(isset($deliveryDataArr['ourLocationId'])){
                    $deliveryCountry .= $deliveryDataArr['ourLocationId']->caption . ": " . $deliveryDataArr['ourLocationId']->value;
                }
                if(isset($deliveryDataArr['deliveryFrom'])){
                    $deliveryCountry .= (!empty($deliveryCountry) ? ', ' : '') . $deliveryDataArr['deliveryFrom']->caption . ": " . $deliveryDataArr['deliveryFrom']->value;
                }

                if(!empty($deliveryCountry)){
                    $body->append($deliveryCountry, 'DELIVERY_COUNTRY');
                }
            }
    ...
    private static function renderDeliveryData($rec, &$tpl)
    {
        if ($Driver = cond_DeliveryTerms::getTransportCalculator($rec->termId)) {
            ...
            foreach ($deliveryDataArr as $delObj){
                $block = clone $tpl->getBlock('DELIVERY_DATA_VALUE');
