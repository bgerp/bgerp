...
class frame2_Reports extends embed_Manager
{
    ...
    protected static function on_AfterPrepareSingleToolbar($mvc, &$data)
    {
        ...
        if ($mvc->haveRightFor('refresh', $rec)) {
            $data->toolbar->addBtn('Обновяване', array($mvc, 'refresh', $rec->id, 'ret_url' => true), 'ef_icon=img/16/arrow_refresh.png,title=Обновяване на справката,onclick=disableBtn(this);');
        }
...
//            wp('Бавно обновяване на отчет', $timer, $rec);
    ...
    {
        $data->listFilter->FLD('user', 'user(rolesForAll=ceo, rolesForTeams=manager|officer, roles=executive, allowEmpty)', 'caption=Потребител,placeholder=Всички');
        $data->listFilter->setField('driverClass', 'placeholder=Всички');
