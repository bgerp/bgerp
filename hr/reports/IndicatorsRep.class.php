...
class hr_reports_IndicatorsRep extends frame2_driver_TableData
{
    ...
    public function addFields(core_Fieldset &$fieldset)
    {
        ...
        $fieldset->FLD('indocators', 'keylist(mvc=hr_IndicatorNames,select=name,allowEmpty)', 'caption=Настройки->Индикатори,placeholderType=all,after=toDate');
        $fieldset->FLD('formula', 'text(rows=2)', 'caption=Настройки->Формула,after=indocators,single=none');
        $fieldset->FLD('personId', 'keylist(mvc=core_Users,select=nick)', 'caption=Настройки->Потребители,placeholderType=all,after=formula,single=none');
    }
