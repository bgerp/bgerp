...
class log_Debug extends core_Manager
{
    ...
    public function act_Default()
    {
        ...
        $data->listFilter->FNC('execTime', 'enum(,fast=< 1 сек.,slow=< 5 сек.,verySlow=< 20 сек., overTime=> 20 сек.)', 'caption=Изпълнение,placeholder=Всички, input, silent,class=debugField');
        $data->listFilter->FNC('execSize', 'enum(,small=< 100K, big=< 300K, veryBig=> 300K, veryBig5=> 500K)', 'caption=Размер,placeholder=Всички, input, silent,class=debugField');
        ...
        $data->listFilter->FNC('status', 'enum(,2xx=Успешен, 8xx=Успешен по AJAX, 000=Неприключен, 150=Наблюдение, 404=Липсваща страница, 500|505|510|0=Икзлючение, 501|520=Грешка, 503=Прекъсване, 550=Грешка в БД)', 'caption=Статус,placeholder=Всички, input, silent,class=debugField');
    ...
    private static function getWpLog($tArr = array(), $dExTime = null, $cookie = null)
    {
        ...
        if (!empty($tArr)) {
            ...
            foreach ($tArr as $rec) {
                ...
                $html .= "\n<li style='word-wrap:break-word;word-break:break-word;hyphens: auto;padding:15px 0px 15px 0px;border-top:solid 1px #cc3;'>" .  number_format(($rec->start), 5) . ': ' . @htmlentities($rec->name, ENT_QUOTES, 'UTF-8');
            }
    ...
    public static function on_AfterSetupMVC($mvc, &$res)
    {
        ...
        if (defined('DEBUG_FATAL_ERRORS_PATH')) {
            $res .= core_Os::createDirectories(DEBUG_FATAL_ERRORS_PATH);
        }
    }
}
