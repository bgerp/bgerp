...
**Инсталацията е автоматична**: при инсталиране на пакета в bgERP, ако програмата не е намерена,
се създава собствен venv в `EF_VENDOR_PATH/markitdown` (по пътя „Без root" по-долу — от името на
уеб потребителя, без sudo) и пътят се записва в `MARKITDOWN_PATH`. Единственото системно
изискване е наличен `python3` >= 3.10 (стандартно за Ubuntu 22.04+). Ръчните варианти по-долу
остават за случаите, когато автоматичната инсталация не е възможна.
