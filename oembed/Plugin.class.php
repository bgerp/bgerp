...
class oembed_Plugin extends core_Plugin
{
    ...
    protected static $oembedMap = array(
        ...
        'National Film Board of Canada' => array(
    ...
    public static function getEmbedHtml($url)
    {
        ...
        if (!$response = static::oembedRequest($apiUrl, $url)) {
     ...
     * 
     * @param string $apiUrl входна точка на oembed сървър
     * @param string $url URL на ресурса, който ще се вгражда
     * @param array $params допълнителни oembed параметри
     */
    protected static function oembedRequest($apiUrl, $url, $params = array())
    {
        $params['url']    = $url;
        $params['format'] = 'json';
        ...
        if (($jsonStr = file_get_contents($requestUrl)) === FALSE) {
            // Нещо не се прочете ... :(
            return FALSE;
        }
