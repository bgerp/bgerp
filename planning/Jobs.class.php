...
class planning_Jobs extends core_Master
{
    ...
    function description()
    {
    	...
    	$this->FLD('department', 'key(mvc=planning_ActivityCenters,select=name,allowEmpty)', 'caption=Цр. дейност');
    	...
    	if($form->isSubmitted()){
    		...
    		if(empty($rec->id)){
    			if(isset($rec->department)){
    				$rec->folderId = planning_ActivityCenters::forceCoverAndFolder($rec->department);
    				unset($rec->threadId);
    			} elseif(empty($rec->saleId)){
    				$rec->folderId = planning_ActivityCenters::forceCoverAndFolder(planning_ActivityCenters::UNDEFINED_ACTIVITY_CENTER_ID);
    				unset($rec->threadId);
    			}
    		}
    ...
    protected static function on_AfterRecToVerbal($mvc, &$row, $rec, $fields = array())
    {
    	...
    	if(isset($fields['-single'])){
    		...
    		if(isset($rec->department)){
    			$row->department = planning_ActivityCenters::getHyperlink($rec->department, TRUE);
    		}
    ...
    public function act_selectTaskAction()
    {
    	...
    	$folderId = (!empty($jobRec->department)) ? planning_ActivityCenters::fetchField($jobRec->department, 'folderId') : NULL;
    ...
    private function getTaskOptions($rec)
    {
    	...
    	$options2 = array();
    	$departments = cls::get('planning_ActivityCenters')->makeArray4Select('name');
    	foreach ($departments as $dId => $dName){
    		$depFolderId = planning_ActivityCenters::fetchField($dId, 'folderId');
    		if(doc_Folders::haveRightToFolder($depFolderId)){
    			$options2["new|{$depFolderId}"] = "В цр. на дейност " . planning_ActivityCenters::getTitleById($dId);
    		}
    	}
