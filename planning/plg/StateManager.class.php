...
class planning_plg_StateManager extends core_Plugin
{
	...
	public static function on_AfterPrepareSingleToolbar($mvc, &$data)
	{
		...
		if($mvc->haveRightFor('close', $rec)){
			$data->toolbar->addBtn("Приключване", array($mvc, 'changeState', $rec->id, 'type' => 'close', 'ret_url' => TRUE), array('ef_icon' => "img/16/lightbulb_off.png",'title' => "Приключване на документа",'warning' => "Сигурнили сте, че искате да приключите документа"));
		}
		 
		// Добавяне на бутон за спиране
		if($mvc->haveRightFor('stop', $rec)){
			$data->toolbar->addBtn("Спиране", array($mvc, 'changeState', $rec->id, 'type' => 'stop', 'ret_url' => TRUE),  array('ef_icon' => "img/16/control_pause.png",'title' => "Спиране на документа",'warning' => "Сигурнили сте, че искате да спрете документа"));
		}
		 
		// Добавяне на бутон за събуждане
		if($mvc->haveRightFor('wakeup', $rec)){
			$data->toolbar->addBtn("Събуждане", array($mvc, 'changeState', $rec->id, 'type' => 'wakeup', 'ret_url' => TRUE),  array('ef_icon' => "img/16/lightbulb.png",'title' => "Събуждане на документа",'warning' => "Сигурнили сте, че искате да събудите документа"));
		}
