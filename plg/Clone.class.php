...
class plg_Clone extends core_Plugin
{
    ...
    {   
        ...
        if ($mvc->haveRightFor('clonerec', $data->rec)) {
            ...
            $data->toolbar->addBtn('Клониране', array($mvc, 'cloneFields', $data->rec->id, 'ret_url' => array($mvc, 'single', $data->rec->id)), "ef_icon=img/16/clone.png,title={$title},row=2,id=clone{$data->rec->id}, order=19.1");
        }
    }
