...
class plg_UserReg extends core_Plugin
{
    ...
    function on_BeforeAction($mvc, &$content, &$act)
    {
        if ($act == 'registernewuser') {
            ...
            if ($rec) {
                // Ако е конфигурирано да се използва е-мейлът за ник,
                // То е-мейлът се записва като Nick
                if (EF_USSERS_EMAIL_AS_NICK) {
                    $rec->nick = $rec->email;
                }
                ...
                if ($eRec = $mvc->fetch("#nick = '{$rec->nick}'")) {
                    if (EF_USSERS_EMAIL_AS_NICK) {
                        if ($eRec->state == 'active') {
                            $form->setError('email', "Вече има регистриран потребител с този е-мейл. Ако сте забравили паролата си, можете да я възстановите тук");
                        } else {
                            $form->setError('email', "Вече има регистриран потребител с този е-мейл. " .
                            "Моля проверете всички папки, в т.ч. ако имате и папката за СПАМ, за е-мейл със заглавие 'Activation'. " .
                            "В него се съдържат инструкции за активиране на вашата сметка. Ако не откриете писмото опитайте да се " .
                            "регистрирате чрез друг ваш е-мейл адрес или направете опит с този след няколко дни.");
                        }
                    } else {
                    ...
                    // проверка дали е-мейлът не се повтаря
                ...
                if (!$form->gotErrors()) {
                    ...
                    // Тук трябва да изпратим е-мейл на потребителя за активиране
            ...
            if (!$form->gotErrors()) {
                ...
                "На посочения от Вас е-мейл ще получите линк за избор на паролата за достъп.");
            }
            ...
            if ($rec) {
                ...
                if (!$id) {
                    sleep(5);
                    $form->setError('email', 'Няма регистриран потребител с този е-мейл');
                } else {
            ...
            "Е-мейл адресът трябва да бъде този, с който сте се регистрирали.|* <br>" .
            "На този е-мейл ще получите линк за избор на нова паролата за достъп.");
