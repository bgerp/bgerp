...
defIfNot('USERREG_ACTIVATION_EMAIL',
    ...
    "\n" .
    "\n|За да активирате акаунта си, моля следвайте долния линк|*:" .
    "\n" .
    "\n[#url#]" .
    "\n" .
            ...
            if ( $form->isSubmitted() && $rec) {
                ...
                if (!$form->gotErrors()) {
                    ...
                    if ($mvc->sendActivationLetter($rec, USERREG_ACTIVATION_EMAIL, 'Активиране на акаунт', 'activate') == FALSE) {
                        redirect(array('Index'), FALSE, tr("За съжеление възникна грешка и не успяхме да изпратим имейла за активиране. Опитайте по-късно или се свържете със системния администратор"), 'error');
                    }
            ...
            if ($form->isSubmitted() && $rec) {
                ...
                if (!$id) {
                    ...
                    if ($mvc->sendActivationLetter($rec, USERREG_RESET_PASS_EMAIL, 'Промяна на парола', 'changePass') == FALSE) {
                        redirect(array('Index'), FALSE, tr("За съжеление възникна грешка и не успяхме да изпратим имейла за възстановяване на паролата. Опитайте по-късно или се свържете със системния администратор"), 'error');
                    }
