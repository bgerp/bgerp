...
class price_Updates extends core_Manager
{
    ...
    {
        $data->listFilter->FLD('classId', 'class(interface=price_CostPolicyIntf,select=title,allowEmpty)', 'caption=Източник,placeholder=Всички');
        $data->listFilter->setFieldType('type', "enum(,category=Категория,product=Артикул,group=Група)");
        $data->listFilter->setField('type', 'input,placeholder=Всички');
