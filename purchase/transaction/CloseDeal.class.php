...
class purchase_transaction_CloseDeal extends deals_ClosedDealTransaction
{
    ...
    public function getTransaction($id)
    {
        ...
        if (Mode::get('saveTransaction')) {
            acc_journal_RejectRedirect::expect(!acc_plg_Contable::haveDocumentInThreadWithStates($rec->threadId, 'pending,draft'), tr("Към покупката има документ в състояние заявка и/или чернова"));
            $rec->valior = $valior;
        }
