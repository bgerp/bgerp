...
class pwa_ProfilePlg extends core_Plugin
{
            ...
            if ($dId) {
                if (pwa_Manifest::canUse($dId) == 'yes') {
                    ...
                    if ($dRec && $dRec->publicKey) {
                        $data->toolbar->addFnBtn('Известяване', '', 'class=pwa-push-default button linkWithIcon, id=push-subscription-button, order=14, title=Абониране за получаване на PUSH известия, row=1');
                    }
                }
            }
        }
    ...
    public static function on_AfterRenderSingle($mvc, &$tpl, $data)
    {
        ...
        if ($key) {
            ...
            $pButton->denied = (object) array('btnText' => tr('Известяване'), 'btnTitle' => tr('От насройките трябва да се разрешат получаването на известия'));
