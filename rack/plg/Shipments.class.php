    ...
    public static function on_AfterGetProductsSummary($mvc, &$res, $rec)
    {
        if(!isset($res)){
            ...
            if(isset($mvc->mainDetail)){
                ...
                $dQuery->EXT('measureId', 'cat_Products', "externalName=measureId,externalKey={$Detail->productFld}");
                $dQuery->XPR('totalQuantity', 'double', "SUM(#{$Detail->quantityFld})");
                $dQuery->groupBy('productId');
                while($dRec = $dQuery->fetch()){
                    $res[$dRec->{$Detail->productFld}] = (object)array('productId' => $dRec->{$Detail->productFld}, 'quantity' => $dRec->totalQuantity, 'measureId' => $dRec->measureId);
                }
            }
        }
    }
