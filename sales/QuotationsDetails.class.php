...
class sales_QuotationsDetails extends doc_Detail {
    ...
    private function groupResultData(&$data)
    {
		...
		if($this->haveRightFor('add', (object)array('quotationId' => $data->masterId))){
    		...
    		$data->addNotOptionalBtn = ht::createBtn('Добавяне',  array($this, 'add', 'quotationId' => $data->masterId, 'optional' => 'no', 'ret_url' => TRUE), FALSE, FALSE, "{$error} ef_icon = img/16/shopping.png, title=Добавяне на офериран артикул към офертата");
