...
class sales_Sales extends deals_DealMaster
{
    ...
    protected static function on_AfterGetPlannedStocks($mvc, &$res, $rec)
    {
        if (is_array($res)) {
            ...
            $manifacturableProducts = array_keys($mvc->getManifacurableProducts($rec));

            // Ако има производими артикули
            if(countR($manifacturableProducts)){
                foreach ($manifacturableProducts as $productId){

                    // и те имат рецепти
                    if($lastReceipt = cat_Products::getLastActiveBom($productId, 'production,instant,sales')) {

                        // За какво количество има пуснати задания за продажбата
                        $receiptClassId = cat_Boms::getClassId();
                        $jobQuery = planning_Jobs::getQuery();
                        $jobQuery->XPR('totalQuantity', 'double', 'SUM(#quantity)');
                        $jobQuery->where("#productId = {$productId} AND #saleId = {$rec->id} AND #state NOT IN ('draft', 'rejected')");
                        $jobQuery->show('id,totalQuantity');
                        $totalQuantity = $jobQuery->fetch()->totalQuantity;

                        // Проверка остава ли още количество за пускане
                        $quantityOrdered = 0;
                        array_walk($res, function($a) use($productId, &$quantityOrdered) { if($a->productId == $productId) {$quantityOrdered += $a->quantityOut;}});
                        $waitingForJob = $quantityOrdered - $totalQuantity;

                        // Ако има извличат се материалите от рецептата
                        if($waitingForJob > 0){
                            $materialArr = cat_Boms::getBomMaterials($lastReceipt, $waitingForJob);

                            // За всеки ще бъде запазен от продажбата
                            foreach ($materialArr as $materialRec){
                                $matpRec = cat_Products::fetch($materialRec->productId, 'generic,canStore');
                                $remainingQuantity = ($materialRec->quantity != cat_BomDetails::CALC_ERROR) ? $materialRec->quantity : 0;
                                if($matpRec->generic == 'yes'){
                                ...
                                $res[] = (object)array('storeId'          => $rec->shipmentStoreId,
