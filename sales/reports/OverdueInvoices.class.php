...
class sales_reports_OverdueInvoices extends frame2_driver_TableData {
	...
	protected function prepareRecs($rec, &$data = NULL) {
		...
		while ( $salesInvoices = $sQuery->fetch () ) {
			...
			if (sales_Sales::fetch ( doc_Threads::getFirstDocument ( $salesInvoices->threadId )->that )->state == 'closed') {
				...
				if (sales_Sales::fetch ( doc_Threads::getFirstDocument ( $salesInvoices->threadId )->that )->closedOn >= $rec->checkDate) {
			...
			foreach ( $threadsId as $thread ) {
				...
				if (is_array ( $invoicePayments )) {
					...
					foreach ( $invoicePayments as $inv => $paydocs ) {
						...
						$iRec = $Invoice->fetch ( 'id,number,dealValue,discountAmount,vatAmount,rate,type,originId,containerId,currencyId,date,dueDate,contragentId' );
	...
	protected function detailRecToVerbal($rec, &$dRec) {
		...
		$row->invoiceNo = ht::createLink ( $invoiceNo, array (
				$dRec->className,
				'single',
				$dRec->invoiceId 
		) );
