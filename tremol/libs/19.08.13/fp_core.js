...
function () {
    ...
    var sendReq = function(verb, address, data) {
        try {
    ...
    var throwOnServerError = function(resp) {
        ...
        if (resCode !== 0) {
            ...
            if(resCode === 40) {
                ...
                throw new Tremol.ServerError(errMsg, resCode, ste1, ste2);
            }
            else {
    ...
    this.do = function(commandName) {
        ...
        try {
            ...
            var response = sendReq("POST", url, xml); */
            ...
            x += "</Command>";
            var response = sendReq("POST", url, x);
    ...
    this.ServerFindDevice = function() {
        w = true;
        try {
            var response = sendReq("GET", url + "finddevice", null);
    ...
    this.ServerGetDeviceSettings = function () {
        w = true;
        try {
            var response = sendReq("GET", url + "settings", null);
    ...
    this.ServerSetDeviceSerialSettings = function (serialPort, baudRate, keepPortOpen) {
        w = true;
        try {
            var response = sendReq("GET", url + "settings(com=" + serialPort +",baud=" + baudRate + ",keepPortOpen=" + (keepPortOpen ? "1" : "0") + ",tcp=0)", null);
            var df = response.getElementsByTagName("defVer");
            if(this.prototype.timeStamp && df.length > 0) {
                ok = (this.prototype.timeStamp === Number(df[0].firstChild.nodeValue));
            }
    ...
    this.ServerSetDeviceTcpSettings = function (ipaddress, tcpport, password) {
        w = true;
        try {
            var response = sendReq("GET", url + "settings(ip=" + ipaddress + ",port=" + tcpport + (password ? (",password=" + password) : "") + ",tcp=1)", null);
            var df = response.getElementsByTagName("defVer");
            if(this.prototype.timeStamp && df.length > 0) {
                ok = (this.prototype.timeStamp === Number(df[0].firstChild.nodeValue));
            }
    ...
    this.ServerGetClients = function () {
        w = true;
        try {
            var response = sendReq("GET", url + "clients", null);
    ...
    this.ServerRemoveClient = function (ip) {
        w = true;
        try {
            sendReq("GET", url + "clientremove(ip=" + ip + ")", null);
        }
        finally {
            w = false;
        }
    ...
    this.ServerCloseDeviceConnection = function () {
        w = true;
        try {
            sendReq("GET", url + "clientremove(who=me)", null);
        }
        finally {
            w = false;
        }
    ...
    this.ServerRemoveAllClients = function () {
        w = true;
        try {
            sendReq("GET", url + "clientremove(who=all)", null);
        }
        finally {
            w = false;
        }
    ...
    this.ServerSetLog = function (enable) {
        w = true;
        try {
            sendReq("GET", url + "log(on=" + (enable ? "1" : "0") + ")", null);
        }
        finally {
            w = false;
        }
 ...
 * @param {number=} ste2
 */
Tremol.ServerError = function (message, type, ste1, ste2) {
    if(!(this instanceof Tremol.ServerError)) {
        return new Tremol.ServerError(message, type, ste1, ste2);
    }
    this.type = type;
