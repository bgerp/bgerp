...
class type_Richtext extends type_Blob
{
     ...
     */
    public static $boldPattern = null;
     ...
     * @return string
     */
    public function removeEmptyLineAfterBlock($text)
    {
     ...
     */
    public static function replaceList($text)
    {
    ...
    public static function getRichTextPatternForBold()
    {
        ...
        if (!isset(self::$boldPattern)) {
            ...
            foreach ($boldTextTypeArr as $boldTextType) {
                ...
                if (!($boldTextType = trim($boldTextType))) {
                    continue;
                }
     ...
     */
    public function _catchBQuote($match)
    {
        // Мястото
        $place = $this->getPlace();
     ...
     */
    public function _catchOneLineCode($match)
    {
    ...
    public function getToolbar(&$attr)
    {
        ...
        if (core_Users::haveRole('user')) {
            ...
            $toolbarArr->add("<span class='popup-table-info'><span class='popupBlock'>" . tr('Колони') . ": <br><input type = 'text' value='5' id='colTable_{$formId}'></span><span class='popupBlock'>" . tr('Редове') .":<br> <input type = 'text' value='3' id='rowTable_{$formId}'/></span><input type='button' id='getTableInfo_{$formId}' onclick=\"createRicheditTable(document.getElementById('{$formId}'), 1, document.getElementById('colTable').value, document.getElementById('rowTable').value );\" value='OK' /> </span>", 'TBL_GROUP2');
            $toolbarArr->add('</span>', 'TBL_GROUP2');
            $toolbarArr->add('</span>', 'TBL_GROUP2');
    ...
    public static function parseInternalUrl($rest)
    {
        ...
        while (isset($restArr[$pId], $restArr[$pId + 1])) {
     ...
     * @return string
     */
    public static function removeEmptyLines($richText, $maxEmptyLines = 2)
    {
        $lines = explode("\n", $richText);
        $empty = 0;
        $newRichText = '';
     ...
     */
    public static function getNotAccessMsg()
    {
